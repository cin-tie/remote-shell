...
import csdev.threads.RemoteShellService;
...
public class RmiClientMain {
    ...
    static class RmiSession{
        ...
        RemoteShellService remoteService;

        RmiSession(String username, String usernameFull, String password, String serverAddress, RemoteShellService remoteService) {
            this.username = username;
            this.usernameFull = usernameFull;
            this.password = password;
            this.serverAddress = serverAddress;
            this.remoteService = remoteService;
            this.sessionId = generateId(username);
        }
    }

    static void session(String username, String usernameFull, String password, String serverAddress, RemoteShellService remoteService) {
        try (Scanner in = new Scanner(System.in)) {
            RmiSession s = new RmiSession(username, usernameFull, password, serverAddress, remoteService);
            if(openSession(s, in)){
                try {
                    displayWelcome(s);
                    while (s.connected) {
            ...
            Logger.logError("RMI Session error: " + e.getMessage());
        }
    }

    static boolean openSession(RmiSession s, Scanner in) {
        Logger.logDebug("Sending RMI connection request...");
        try {
            MessageConnect messageConnect = new MessageConnect(s.username, s.usernameFull, s.password);
            MessageConnectResult msg = s.remoteService.connect(messageConnect, s.sessionId);

            if (!msg.Error()) {
                s.connected = true;
                s.serverOS = msg.serverOS;
                s.currentDirectory = msg.currentDir;
                Logger.logInfo("Connected via RMI to server: " + msg.serverOS);
                Logger.logInfo("Current directory: " + msg.currentDir);
                Logger.logInfo("Server version: " + msg.serverVersion);
                return true;
            }

            Logger.logError("Unable to connect via RMI: " + msg.getErrorMessage());
        } catch (Exception e) {
            Logger.logError("RMI Connection error: " + e.getMessage());
        }
    ...
    static void closeSession(RmiSession s) {
        if(s.connected) {
            s.connected = false;
            try {
                MessageDisconnect disconnect = new MessageDisconnect("Client shutdown");
                s.remoteService.disconnect(disconnect, s.sessionId);
            } catch (Exception e) {
                Logger.logError("RMI sending disconnect error: " + e.getMessage());
            }
            Logger.logInfo("Disconnected from RMI server");
        }
    }
    ...
    static Message getCommand(RmiSession ses, Scanner in) {
        while (true) {
            printPrompt(ses);
            if (!in.hasNextLine())
                break;
    ...
    static void printPrompt(RmiSession s) {
    ...
    static private String generateId(String username){
        return "RMI_" + username  + "_" + System.currentTimeMillis();
    }
}
